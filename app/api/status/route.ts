...
export async function GET() {
  ...
  const connection = user ? await db.platformConnection.findUnique({ where: { userId_platform: { userId: user.id, platform: "instagram" } } }) : null;
  ...
  return NextResponse.json({ portfolio: agents.some((agent) => agent.state !== "RUNNING") ? "ATTENTION" : "RUNNING", agents, pending, mode: connection?.mode ?? "DRY_RUN", connected: connection?.status === "CONNECTED", agentGateway: { ...await getAgentGatewayStatus(user.id), lastDecision } }, { headers: { "cache-control": "no-store" } });
}
