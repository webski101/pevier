...
export async function GET() {
  ...
  if (!user) return NextResponse.json({ channels: [], agents: [] });
  const connection = await db.platformConnection.findUnique({ where: { userId_platform: { userId: user.id, platform: "youtube" } } });
